...
if (isset($_POST["add-user"])) {
  try {
    ...
    if (count($permissions) === 0) {
      throw new ClientException("At least one permission must be selected");
    }
    ...
    $user->username = trim(strtolower($_POST["username"])) ?: $user->makeUsername();
    $user->email = $user->username . "@v.je";
    $user->password = $password;
    $user->is_admin = true;
    $user->perm = $perm;

    if (User::exists(["username" => $user->username, "email" => $user->email], Condition::OR)) {
      throw new ClientException("User with this username or email already exists, use the username fields to override the default username.");
    }
  ...
  try {
    ...
    if ($action === "remove-priv") {
      ...
      throw new ClientException("Invalid action");
    }
...
<div class="md:flex w-full gap-6 mt-4">
  ...
  <form class="w-full md:w-1/2 lg:w-1/3" method="POST">
    <div class="form-control">
      ...
      <input type="text" id="first_name" name="first_name" placeholder="john" class="w-full" value="<?php echo $prev['first_name'] ?? '' ?>" required />
    </div>

    <div class="form-control">
      <label for="last_name">Last name</label>
      <input type="text" id="last_name" name="last_name" placeholder="doe" class="w-full" value="<?php echo $prev['last_name'] ?? '' ?>" required />
    </div>

    <!-- Optional to override the default username that will be generated by the system -->
    <div class="form-control">
      <label for="username">Username</label>
      <input type="text" id="username" name="username" placeholder="jdoe" class="w-full" value="<?php echo $prev['username'] ?? '' ?>" />
    </div>

    <div class="form-control">
      <label for="password">Password</label>
      <input type="password" autocomplete="new-password" id="password" name="password" placeholder="******" minlength="6" maxlength="24" class="w-full" requuired />
    </div>

    <div class="form-control">
      <label for="confirm_password">Confirm Password</label>
      <input type="password" id="confirm_password" name="confirm_password" placeholder="******" minlength="6" maxlength="24" class="w-full" required />
    </div>
